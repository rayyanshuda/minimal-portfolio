...
export function generateStaticParams() {
  return loadContentPieces("machine-learning-journey").map((piece) => ({ slug: piece.id }));
}

export default async function MlJourneyPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <RhPageShell
      ...
      contentsItems={pieces.map((p, index) => ({
        ...
        href: `/blog/machine-learning/${p.id}`,
      }))}
    >
      <RhSubpageHeader
        title={piece.title}
